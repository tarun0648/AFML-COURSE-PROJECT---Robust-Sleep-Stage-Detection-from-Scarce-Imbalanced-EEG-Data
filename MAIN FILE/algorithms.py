...
class vae(Algorithm):
    """VAE algorithm for generating synthetic EEG data"""
    def __init__(self, vae_model, configs, hparams, device):
        super(vae, self).__init__(configs)
        ...
        self.hparams = hparams
        self.beta = hparams.get("beta", 1.0)  # Beta for beta-VAE
    ...
    def update(self, samples):
        ...
        reconstruction, mu, logvar = self.vae_model(data)
        
        # Calculate VAE loss
        from models.models import vae_loss_function
        total_loss, recon_loss, kl_loss = vae_loss_function(
            reconstruction, data, mu, logvar, beta=self.beta
        )
