...
def data_generator(data_path, data_type, fold_id, data_percentage, dataset_configs, hparams, train_mode, ssl_method,
    ...
    train_dataset = torch.load(
        os.path.join(data_path, data_type, f"train_{fold_id}_{data_percentage}per.pt"), weights_only=False)
    val_dataset = torch.load(os.path.join(data_path, data_type, f"val_{fold_id}.pt"), weights_only=False)
...
def calc_class_weight(labels_count):
    ...
    for key in range(num_classes):
        score = math.log(mu[key] * total / float(labels_count[key], weights_only=False))
