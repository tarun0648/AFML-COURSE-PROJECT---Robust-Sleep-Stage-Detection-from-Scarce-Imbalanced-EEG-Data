...
##########################################################################################
# VAE for EEG Data Generation
##########################################################################################

class VAE_EEG(nn.Module):
    """
    Variational Autoencoder for EEG signal generation.
    Designed to generate synthetic minority-class (N1/N3) EEG samples.
    """
    def __init__(self, configs, latent_dim=128):
        super(VAE_EEG, self).__init__()
        ...
        self.encoder = nn.Sequential(
            nn.Conv1d(self.input_channels, 32, kernel_size=25, stride=2, padding=12),
            nn.BatchNorm1d(32),
            nn.LeakyReLU(0.2),
            nn.Dropout(0.2),
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose1d(256, 128, kernel_size=10, stride=2, padding=4, output_padding=1),
            nn.BatchNorm1d(128),
            nn.LeakyReLU(0.2),
            nn.Dropout(0.2),
            ...
            nn.ConvTranspose1d(32, self.input_channels, kernel_size=25, stride=2, padding=12, output_padding=1),
            nn.Tanh()  # Output in range [-1, 1]
        )
        
    def _get_flatten_size(self):
        """Calculate the flattened size after encoder convolutions"""
        dummy_input = torch.zeros(1, self.input_channels, self.sequence_len)
    ...
    def decode(self, z):
        ...
        if reconstruction.size(2) != self.sequence_len:
            reconstruction = nn.functional.interpolate(reconstruction, size=self.sequence_len, mode='linear', align_corners=False)
    ...
    def generate_from_class(self, num_samples, class_latent_mean, class_latent_std, device='cuda'):
        """Generate samples from class-specific latent distribution"""
        self.eval()
        with torch.no_grad():
            z = torch.randn(num_samples, self.latent_dim).to(device) * class_latent_std + class_latent_mean
            samples = self.decode(z)
        return samples
